...
public interface APISERVER {
    ...
    @FormUrlEncoded
    @POST("Checkdaily/")
    Call<POJO_Checkdaily> getDatadaily(@Field("name") String name, @Field("score") int score, @Field("date") String date);
    ...
    @FormUrlEncoded
    @POST("assessmentstu-1/")
    Call<POJO_PJ_P1> getDataPJP1_EIL(@Field("member_id") int first, @Field("ex11") int Sec,
                                     @Field("ex12") int Thrir, @Field("ex13") int four, @Field("ex14") int five,
                                     @Field("ex15") int six);
...
//    Call<POJO_AG> getDatastuAG (@Field("member_id ") int first);
    ...
    @GET("em-api/")
    Call<List<POJO_trainer>> get_trainer(@Field("type") String first);
    ...
    @FormUrlEncoded
    @POST("apprentice/")
    Call<List<POJO_getstuemp>> get_getstuemp(@Field("dep_id") String first ,@Field("date") String date );


    @FormUrlEncoded
    @POST("save-data/")
    Call<List<POJO_save_data_trainer>> get_save_data_trainer(@Field("member_id") int first, @Field("check_dep") int check_nited , @Field("date") String date ,@Field("type") String four);


    @FormUrlEncoded
    @POST("save-cf/")
    Call<List<POJO_confirm_AG>> get_comfirm_ag(@Field("dep_id") int first, @Field("date") String date);

    @FormUrlEncoded
    @POST("save-ag/")
    Call<List<POJO_save_AG>> get_save_ag(@Field("member_id") int first, @Field("check_dep") int sec ,@Field("score") int Thrir , @Field("date") String date ,@Field("type") String four);

    @FormUrlEncoded
    @POST("getdatatest/")
    Call<List<POJO_getdata_admin>> getdata_admin(@Field("type") String first);

    @FormUrlEncoded
    @POST("getdate-admin/")
    Call<List<POJO_getdate_admin>> getdate_admin(@Field("dep_id") String first);

    @FormUrlEncoded
    @POST("getdata-parent-member/")
    Call<List<POJO_getdata_parent_member>> getdate_parent_member(@Field("member_id") int first);

    @FormUrlEncoded
    @POST("getdate-admin-eli/")
    Call<List<POJO_getdate_eil>> getdate_eil(@Field("dep_id") String first);

    @FormUrlEncoded
    @POST("del-apprentice/")
    Call<POJO_Del> Del_data (@Field("app_id") String first);
